...
async function main() {
  ...
  await prisma.supportWorker.createMany({ data: workers, skipDuplicates: true });
  console.log("✅ Support workers created!");

  // Mirror support workers into users table (role=support_worker) so legacy FK 'worker_id' -> users.id is satisfied
  console.log("🧩 Ensuring matching user records for support workers (role=support_worker)...");
  for (const w of workers) {
    await prisma.user.upsert({
      ...
      create: {
        clerk_user_id: `support-worker:${w.email}`,
        first_name: w.first_name,
        last_name: w.last_name,
        email: w.email,
        role: "support_worker",
  ...
  console.log("✅ Support workers mirrored into users table!");
