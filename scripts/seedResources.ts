/**
 * Seed script to populate Convex resources database with hardcoded mental health resources
 * Run this script to initialize the resources in your Convex backend
 * 
 * Usage: npx tsx scripts/seedResources.ts
 */
...
if (!CONVEX_URL) {
  ...
  console.error("Current env keys:", Object.keys(process.env).filter(k => k.includes('CONVEX')));
  process.exit(1);
}
...
// Complete offline resource database - matching utils/resourcesApi.ts
const ALL_RESOURCES = [
...
async function seedResources() {
  try {
    ...
    // Seed resources using the Convex mutation
    const result = await client.mutation(api.resources.seedResources, {
